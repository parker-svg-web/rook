...
const styles = `
  ...
  .feature-card:hover { transform: translateY(-2px); box-shadow: 0 8px 30px rgba(37, 99, 235, 0.1); border-color: rgba(56, 189, 248, 0.3) !important; }
  .cta-btn { transition: all 0.2s ease; }
  .cta-btn:hover { transform: translateY(-1px); box-shadow: 0 8px 25px rgba(37, 99, 235, 0.4); }
  .ghost-btn { transition: all 0.2s ease; }
  .ghost-btn:hover { border-color: #38bdf8 !important; color: #38bdf8 !important; }
  ...
  .dot-pattern { background-image: radial-gradient(rgba(56,189,248,0.07) 1px, transparent 1px); background-size: 24px 24px; }
  .pricing-card { transition: all 0.3s ease; }
  .pricing-card:hover { transform: translateY(-4px); box-shadow: 0 20px 40px rgba(0,0,0,0.3); }
  .popular-card { transition: all 0.3s ease; box-shadow: 0 0 30px rgba(37, 99, 235, 0.25); }
  .popular-card:hover { transform: translateY(-4px); box-shadow: 0 0 50px rgba(37, 99, 235, 0.4); }
`;

export default function LandingPage({ onGetStarted, onSignIn }: LandingPageProps) {
  return (
    <div style={{ fontFamily: 'Inter, system-ui, -apple-system, sans-serif', color: '#f1f5f9', minHeight: '100vh', position: 'relative' }} className="animated-bg dot-pattern">
      <style>{styles}</style>
      <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&display=swap" rel="stylesheet" />

      {/* Hero radial glow */}
      <div style={{ position: 'absolute', top: '-20%', left: '50%', transform: 'translateX(-50%)', width: '800px', height: '800px', background: 'radial-gradient(circle, rgba(56,189,248,0.08) 0%, transparent 70%)', pointerEvents: 'none', zIndex: 0 }} />
      ...
      <nav style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 48px', borderBottom: '1px solid rgba(255,255,255,0.06)', backdropFilter: 'blur(12px)', position: 'sticky', top: 0, zIndex: 100, background: 'rgba(15,23,42,0.8)' }}>
        ...
        <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
          <a href="#features" style={{ color: '#94a3b8', textDecoration: 'none', fontSize: 14, transition: 'all 0.2s ease' }} onMouseEnter={e => (e.target as HTMLElement).style.color = '#e2e8f0'} onMouseLeave={e => (e.target as HTMLElement).style.color = '#94a3b8'}>Features</a>
          <a href="#pricing" style={{ color: '#94a3b8', textDecoration: 'none', fontSize: 14, transition: 'all 0.2s ease' }} onMouseEnter={e => (e.target as HTMLElement).style.color = '#e2e8f0'} onMouseLeave={e => (e.target as HTMLElement).style.color = '#94a3b8'}>Pricing</a>
          <button onClick={onSignIn} className="ghost-btn" style={{ background: 'transparent', border: '1px solid #334155', borderRadius: 8, padding: '8px 20px', color: '#e2e8f0', cursor: 'pointer', fontSize: 14 }}>Sign In</button>
          <button onClick={onGetStarted} className="cta-btn" style={{ background: '#2563eb', border: 'none', borderRadius: 8, padding: '8px 20px', color: 'white', cursor: 'pointer', fontWeight: 600, fontSize: 14 }}>Get Started</button>
        </div>
      </nav>

      {/* Hero */}
      <section className="hero-section" style={{ maxWidth: 1000, margin: '0 auto', padding: '100px 48px', textAlign: 'center', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ display: 'flex', gap: 16, justifyContent: 'center' }}>
          <button onClick={onGetStarted} className="cta-btn" style={{ background: '#2563eb', border: 'none', borderRadius: 12, padding: '16px 40px', color: 'white', cursor: 'pointer', fontWeight: 600, fontSize: 18 }}>
            Start Free Trial
          </button>
          <a href="#pricing" className="ghost-btn" style={{ background: 'transparent', border: '1px solid #334155', borderRadius: 12, padding: '16px 40px', color: '#e2e8f0', cursor: 'pointer', fontWeight: 500, fontSize: 18, textDecoration: 'none', display: 'inline-block' }}>
            View Plans
          </a>
        </div>
      ...
      {/* Features */}
      <section id="features" style={{ maxWidth: 1100, margin: '0 auto', padding: '80px 48px', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: 24 }}>
          {[
            ...
            <div key={f.title} className="feature-card" style={{ background: '#1e293b', borderRadius: 16, padding: 32, border: '1px solid #334155' }}>
      ...
      {/* Pricing */}
      <section id="pricing" style={{ maxWidth: 1000, margin: '0 auto', padding: '80px 48px', position: 'relative', zIndex: 1 }}>
      ...
      {/* CTA */}
      <section style={{ maxWidth: 700, margin: '0 auto', padding: '80px 48px', textAlign: 'center', position: 'relative', zIndex: 1 }}>
        <div style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: '600px', height: '200px', background: 'radial-gradient(circle, rgba(37,99,235,0.1) 0%, transparent 70%)', pointerEvents: 'none' }} />
        <h2 style={{ fontSize: 36, fontWeight: 700, margin: '0 0 16px', color: '#f1f5f9' }}>One plan. All your APIs.</h2>
        <p style={{ color: '#94a3b8', fontSize: 18, marginBottom: 32 }}>Join companies that cut their API vendor management overhead by 80%.</p>
        <button onClick={onGetStarted} className="cta-btn" style={{ background: '#2563eb', border: 'none', borderRadius: 12, padding: '16px 48px', color: 'white', cursor: 'pointer', fontWeight: 600, fontSize: 18 }}>
          Get Started Free
        </button>
      </section>

      {/* Footer */}
      <footer style={{ borderTop: '1px solid #1e293b', padding: '32px 48px', textAlign: 'center', color: '#475569', fontSize: 13, position: 'relative', zIndex: 1 }}>
